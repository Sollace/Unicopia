...
record Recipe (DynamicContent.Page page, Identifier id, Bounds bounds) implements PageElement {
    ...
    public void compile(int y, IViewRoot container) {
        MinecraftClient.getInstance().world.getRegistryManager().getOrThrow(RegistryKeys.RECIPE).getOptionalValue(id).ifPresent(recipe -> {
            if (recipe instanceof SpellbookRecipe spellRecipe) {
                ...
                bounds.height = 0;
                spellRecipe.getDisplays().stream()
                    .filter(display -> display instanceof SpellbookRecipeDisplay)
                    .forEach(display -> {
