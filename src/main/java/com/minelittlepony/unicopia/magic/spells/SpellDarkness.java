...
public class SpellDarkness extends AbstractAttachableSpell {
    ...
    public boolean update(ICaster<?> source) {
        ...
        if (hasTarget()) {
            ...
            source.getEntity().getVelocity().y -= 0.01;
    ...
    private void applyDark(ICaster<?> source, LivingEntity entity) {
        ...
        if (!isLightholder(entity)) {
            ...
            if (distance < 3 && entity.world.random.nextInt(30) == 0) {
                entity.damage(MagicalDamageSource.create("darkness"), 3);
            }
    ...
    public void spawnSphere(ICaster<?> source, Vec3d pos, int tint, int maxSize) {
        ...
        if (size > 0) {
            ...
            ParticleTypeRegistry.getTnstance().spawnParticle(UParticles.SPHERE, true, pos, vX, 0, vZ, (int)(size * 1000), tint, 30);
        }
    }
