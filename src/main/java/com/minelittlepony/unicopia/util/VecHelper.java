...
public interface VecHelper {
    ...
    static Vec3d lerp(float delta, Vec3d a, Vec3d b) {
        return new Vec3d(
    ...
    static Vec3d gaussian(Random random) {
        return supply(() -> random.nextGaussian());
    }

    static Vec3d triangular(Random random, double mode, double deviation) {
        return supply(() -> random.nextTriangular(mode, deviation));
    }
