...
public class AirBalloonEntity extends FlyingVehicleEntity {
    ...
    protected Box calculateBoundingBox() {
        Box mainBox = super.calculateBoundingBox();
    ...
    public Box getVisibilityBoundingBox() {
        if (hasBalloon()) {
            return getBalloonBoundingBox(getBoundingBox()).withMinY(getY());
        }
        return getInteriorBoundingBox(getBoundingBox());
    }

    protected Box getInteriorBoundingBox(Box mainBox) {
        Box box = MultiBox.unbox(mainBox);
        return box.withMinY(box.minY - 0.5).contract(0.15, 0, 0.15);
    }

    protected Box getBalloonBoundingBox(Box mainBox) {
