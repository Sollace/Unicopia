...
public interface TossableItem extends Tossable<ItemStack>, Dispensable {
    ...
    @Nullable
    default Projectile createProjectile(World world, PlayerEntity player) {
        return new MagicProjectileEntity(null, world, player);
    }

    @Nullable
    default Projectile createProjectile(World world, Position pos) {
        return new MagicProjectileEntity(null, world, pos.getX(), pos.getY(), pos.getZ());
    }
    ...
    default void toss(World world, ItemStack itemstack, PlayerEntity player) {
        ...
        if (!world.isClient) {
            Projectile projectile = createProjectile(world, player);
    ...
    default ItemStack toss(World world, Position pos, Direction facing, ItemStack stack) {
        Projectile projectile = createProjectile(world, pos);
