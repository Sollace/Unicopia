...
public class CucoonEntity extends LivingEntity implements Magical, InAnimate, Trap {
    ...
    public void tick() {
        ...
        if (passenger != null) {
            ...
            if (passenger instanceof LivingEntity) {
                ...
                if (!living.hasStatusEffect(StatusEffects.REGENERATION) && living.getHealth() < living.getMaxHealth()) {
                    living.addStatusEffect(new StatusEffectInstance(StatusEffects.REGENERATION, 20, 2));
                }

                if (!living.hasStatusEffect(StatusEffects.SLOWNESS) && living.getHealth() < living.getMaxHealth()) {
    ...
    public ActionResult interactAt(PlayerEntity player, Vec3d vec, Hand hand) {
        ...
        if (hand == Hand.MAIN_HAND && EquinePredicates.PLAYER_CHANGELING.test(player)) {
            ...
            if (passenger != null && (player.canConsume(false) || player.getHealth() < player.getMaxHealth())) {
    ...
    protected void updatePostDeath() {
        if (++deathTime == 20) {
            if (!world.isClient && lastAttackedTicks > 0 && canDropLootAndXp() && world.getGameRules().getBoolean(GameRules.DO_ENTITY_DROPS)) {
