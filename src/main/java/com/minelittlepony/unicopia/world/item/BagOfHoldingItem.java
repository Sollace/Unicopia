...
import com.minelittlepony.unicopia.world.container.BagOfHoldingInventory;
...
public class BagOfHoldingItem extends Item implements AffineItem {
    ...
    public void appendTooltip(ItemStack stack, @Nullable World worldIn, List<Text> tooltip, TooltipContext flagIn) {
        ...
        counts.forEach((line, count) -> {
            tooltip.add(line.copy().append(" ").append(count.toString()));
        });
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity player, Hand hand) {
        ...
        if (player.isSneaking()) {
            ...
            if (hit != null) {
                if (hit.getType() == HitResult.Type.BLOCK) {
                    ...
                    if (e instanceof LivingEntity && !(e instanceof PlayerEntity) && !(e instanceof Magical)) {
                        if (!world.isClient) {
                            BagOfHoldingInventory inventory = BagOfHoldingInventory.getInventoryFromStack(stack);
        ...
        player.openHandledScreen(new SimpleNamedScreenHandlerFactory(
                (sync, inv, ply) -> new BagOfHoldingContainer(sync, inv),
                stack.hasCustomName() ? stack.getName() : new TranslatableText("item.unicopia.bag_of_holding")));
