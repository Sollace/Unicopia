...
import com.minelittlepony.unicopia.equine.player.MagicReserves;
...
public class AlicornAmuletItem extends ArmorItem implements AffineItem, ItemImpl.TickableItem {
    ...
    public void inventoryTick(ItemStack stack, World world, Entity entity, int slot, boolean selected) {
        ...
        if (attachedTime > 0 && attachedTime % 100 == 0) {
            world.playSound(null, player.getBlockPos(), SoundEvents.AMBIENT_NETHER_WASTES_MOOD, SoundCategory.PLAYERS, 3, 1);
        }
    ...
    public void onRemoved(Pony player, float needfulness) {
        ...
        if (attachedTime > 120) {
            player.getOwner().takeKnockback(1, 1, 1);
        }
    }
    ...
    public Multimap<EntityAttribute, EntityAttributeModifier> getAttributeModifiers(EquipmentSlot equipmentSlot) {
        ...
        if (equipmentSlot == slot) {
            ...
            multimap.put(EntityAttributes.GENERIC_ATTACK_DAMAGE, new EntityAttributeModifier(modifierId, "Strength modifier", 50, EntityAttributeModifier.Operation.ADDITION));
        }
