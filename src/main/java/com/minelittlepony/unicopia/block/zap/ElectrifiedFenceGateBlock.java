...
public class ElectrifiedFenceGateBlock extends FenceGateBlock implements ElectrifiedBlock {
    private static final MapCodec<ElectrifiedFenceGateBlock> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
            WoodType.CODEC.fieldOf("wood_type").forGetter(block -> block.type),
            createSettingsCodec()
    ).apply(instance, ElectrifiedFenceGateBlock::new));

    private final WoodType type;

    public ElectrifiedFenceGateBlock(WoodType type, Settings settings) {
        super(type, settings);
        this.type = type;
    }
